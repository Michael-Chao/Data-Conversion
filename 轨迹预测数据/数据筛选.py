...
#     result_adsb_data = f.readlines()
#
# # 对每个filter_ADSB数据项补充"GS"和"TA"
# for i, item in enumerate(filter_adsb_data):
#     for line in result_adsb_data:
#         line_dict = dict(kv.split("=") for kv in line.strip('{}').split(', '))
#         if "TRP" in line_dict and "ADDR" in line_dict:
#             if float(item["TRP"]) == float(line_dict["TRP"]) and item["ADDR"] == line_dict["ADDR"]:
#                 filter_adsb_data[i]['GS'] = float(line_dict.get("GS", 0))
#                 filter_adsb_data[i]['TA'] = float(line_dict.get("TA", 0))
#                 break
#
# # 写入更新后的数据到新文件夹
# output_file_path = os.path.join(output_folder, os.path.basename(filter_adsb_file_path))
# with open(output_file_path, 'w') as f:
#     json.dump(filter_adsb_data, f)
#
...
# 定义文件夹路径
result_adsb_folder_path = r'D:\Project\Convert_Dataset\轨迹预测数据\result_ADSB'
